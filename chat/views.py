...
class MockTextChatbot:
    ...
    def predict_answer(self, query): 
        ...
        all_answers = {
             # --- POSH ---
            "definitions & scope": "POSH Act covers sexual harassment at workplace. It defines 'aggrieved woman', 'workplace', and 'sexual harassment'.",
            "the internal committee (ic)": "Every employer of a workplace must constitute an Internal Committee (IC) if there are 10 or more employees.",
            "filing a complaint": "An aggrieved woman can file a complaint in writing to the IC within 3 months from the date of the incident.",
            "inquiry & procedure": "The IC must complete the inquiry within 90 days. Both parties must be given a fair opportunity to be heard.",
            "conciliation & settlement": "Before initiating inquiry, the IC may, at the request of the aggrieved woman, take steps to settle the matter through conciliation.",
            "recommendations & action": "On completion of inquiry, the IC provides a report. If allegations are proved, it recommends action against the respondent.",
            "employer obligations": "Employers must provide a safe working environment, display penal consequences of sexual harassment, and organize workshops.",
            "scenario-based \"grey areas\"": "Grey areas include consensual relationships turned sour, harassment outside office hours but impactful on work, etc.",
            
            # --- POSCO ---
            "legal core & definitions": "POSCO Act 2012 provides protection to children from offenses of sexual assault, sexual harassment and pornography.",
            "mandatory reporting (sec 19)": "Section 19 mandates reporting of sexual offenses against children to the Special Juvenile Police Unit or local police.",
            "aggravated offences": "Aggravated penetrative sexual assault includes offenses by persons in authority (police, armed forces, public servants) and carries stricter punishment.",
            "child pornography & digital": "Using children for pornographic purposes or storing such material is a serious offense under POSCO.",
            "investigation procedures": "Investigation must be conducted by a child-friendly officer, not in uniform, and the child should not be detained at the police station.",
            "medical examination": "Medical examination of the child must be conducted in the presence of parents/guardians and by a female doctor if possible.",
            "special courts & trial": "Special Courts are designated for speedy trial of POSCO cases. The trial should be completed within one year.",
            "institutional safeguards": "Requires child care institutions, schools, and hostels to have guidelines and mechanisms to prevent abuse.",

            # --- IC ---
            "composition & legal status": "IC must have a Presiding Officer (senior woman), 2 employee members committed to women's cause/legal knowledge, and 1 external member.",
            "the external member": "External Member must be from an NGO or association committed to the cause of women or a person familiar with issues relating to sexual harassment.",
            "pre-inquiry procedures": "IC meets to review the complaint, ensures copies are given to the respondent within 7 days, and checks limitation period.",
            "principles of natural justice": "Both parties have right to be heard. No bias. Cross-examination is allowed dependent on IC discretion.",
            "evidence & documentation": "IC follows civil court powers for summoning witnesses, discovering documents. Confidentiality is key.",
            "the inquiry report": "Report must include findings and recommendations. Must be submitted to employer within 10 days of completion.",
            "punishments & remedies": "Punishments range from written apology, withholding promotion/increment, to termination. Compensation can be deducted from salary.",
            "post-inquiry & compliance": "Employer must act on recommendations within 60 days. Annual Report inclusion is mandatory.",

            # --- Mental Health ---
            "mental health literacy": "Understanding signs of stress, burnout, anxiety, and depression in the workplace context.",
            "legal rights & compliance": "Mental Healthcare Act 2017 ensures right to equality and non-discrimination for persons with mental illness.",
            "psychological safety": "Creating an environment where employees feel safe to express ideas, questions, and concerns without fear of punishment.",
            "managerial responsibilities": "Managers should be trained to identify distress, offer support, and not stigmatize mental health issues.",
            "work-life integration": "Policies supporting flexible hours, leave for mental health, and respecting boundaries after work hours.",
            "stigma & language": "Avoid pejorative terms. Use person-first language. Promote open conversations about mental health.",
            "crisis intervention": "Protocols for handling acute mental health episodes (panic attacks, suicidal ideation) at the workplace.",
            "scenario-based challenges": "Handling an employee's return to work after mental health leave; Managing performance issues linked to mental health."
        }
        ...
        return f"I received your question: '{query}'. However, the AI models are not loaded in this environment, so this is a placeholder response."
...
if image_chatbot is None: image_chatbot = MockImageChatbot()
if text_chatbot is None: text_chatbot = MockTextChatbot()
...
def chatbot_response(request):
    ...
    if question_lower in irc_map:
        ...
        questions = []
        if image_chatbot:
             images = format_images_for_response(image_chatbot.get_images_by_irc(irc_type, limit=4))
        if text_chatbot:
    ...
    if llm_response and "answering using context" not in llm_response.lower() and "model is not ready" not in llm_response.lower():
         return JsonResponse({
            "response": {
