    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            if "clean_name" not in self.data.columns:
                raise ValueError(
                    "Model is outdated. Please retrain with the new script to create 'clean_name'."
                )

            model_version = self.model_data.get("model_version", "Unknown")
            trained_at = self.model_data.get("trained_at", "Not specified")
            print(
                f"Image Chatbot Model loaded successfully! Version: {model_version} (Trained at: {trained_at})"
            )

            # Print available IRC codes
            if self.irc_index:
                print(
                    f"Available IRC codes for images: {', '.join(sorted(self.irc_index.keys()))}"
                )
...
class TextChatbot:
    def __init__(self, model_path, label_encoder_path, semantic_data_path):
        ...
        try:
            ...
            raise ValueError(
                "Model files are outdated! Please re-run your training script to include 'types' and 'keywords' in 'semantic_data.pkl'."
            )
    ...
    def get_semantic_match_with_keyword_ranking(self, user_question, threshold=0.60):
        ...
        if candidates:
            ...
            print(
                f"   Score: {best['score']:.2f} | Keywords in question: {best['keyword_count']} | Stored keywords: {best['keywords']}"
            )
    ...
    def predict_answer(self, user_question, suggestion_threshold=0.60):
        ...
        if top_score >= suggestion_threshold:
            ...
            return f"We regret that your query falls outside RoadAthena scope; Our AI chatbot is continually improving to broaden its capabilities, Did you mean one of these?\n\n{options}\n\n"
